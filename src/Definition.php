...
class Definition
{
     ...
     * For classes, this is the class type (object).
     * Variables are not indexed for performance reasons.
     * Can also be a compound type.
     * If it is unknown, will be Types\Mixed.
     * If the declaration does not have a type (like a trait) it will be null.
     *
    ...
    public static function getTypeFromNode(Node $node)
    {
        if ($node instanceof Node\Stmt\Class_ || $node instanceof Node\Stmt\Interface_) {
            ...
            if ($node instanceof Node\Stmt\Class_ && $node->isAnonymous()) {
                // Anoynmous classes resolve to an object type
                return new Types\Object_;
            }
            // Classes resolve to the class type
            return new Types\Object_(new Fqsen('\\' . (string)$node->namespacedName));
        }
        if ($node instanceof Node\FunctionLike) {
            ...
            if ($node->returnType !== null) {
                ...
                if (is_string($node->returnType)) {
                    ...
                    return (new TypeResolver)->resolve((string)$node->returnType);
                }
